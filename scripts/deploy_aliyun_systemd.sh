...
ALLOWED_ORIGINS="${TONGPIN_ALLOWED_ORIGINS:-*}"
SSH_KEY="${SSH_KEY:-}"
SSH_OPTS="${SSH_OPTS:-}"
...
ssh "${SSH_ARGS[@]}" "${SERVER_USER}@${SERVER_HOST}" \
  "APP_DIR='${APP_DIR}' APP_HOST='${APP_HOST}' APP_PORT='${APP_PORT}' ADMIN_EMAIL='${ADMIN_EMAIL}' ADMIN_PASSWORD='${ADMIN_PASSWORD}' SUPPORT_WECHAT='${SUPPORT_WECHAT}' SUPPORT_HOURS='${SUPPORT_HOURS}' SUPPORT_MESSAGE='${SUPPORT_MESSAGE}' COOKIE_SECURE='${COOKIE_SECURE}' ALLOWED_ORIGINS='${ALLOWED_ORIGINS}' bash -s" <<'REMOTE'
set -euo pipefail
...
TONGPIN_ALLOWED_ORIGINS=${ALLOWED_ORIGINS}
